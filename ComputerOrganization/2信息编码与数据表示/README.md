...
进位计数法是一种计数的方法，生活中常用10进制数表示，计算机内常用2进制、8进制、16进制表示数据。在进位计数法中，每个数位可能用到的不同数码个数称为**基数**，十进制基数为10，每个数位满10就向高位进1。进位计数法中每一位所表示的权值不同，比如十进制的123，第一位的权值是100，第二位是10，第三位是1，111表示的数就是1\*100+2\*10+3\*1 = 123。
...
<img src="./assets/image-20180914140003457.png" width=400>
...
阶码是整数，阶符J<sub>s</sub>和阶码的位数m合起来反应的是浮点数表示范围和小数点的实际位置；数符S<sub>s</sub>代表浮点数的符号，位数的位数n反应浮点数的精度。
